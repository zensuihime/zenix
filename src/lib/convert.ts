...
import type { ConvertOptions, ProcessingResult, SupportedFormat } from '../types';
...
async function processDirectory(
    ...
    for (const file of files) {
        try {
            ...
            errorCount++;
            const relativePath = path.relative(inputDir, file);
            const errorMessage = `Error converting ${relativePath}: ${error instanceof Error ? error.message : String(error)}`;
